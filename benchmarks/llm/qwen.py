...
SPLIT_DESC_CSV_SAVE_PATH = '../output/qwen_prediction.csv'
ERDES = '/erdes'
...
def generate_video_summary(record, extracted_frames):
    ...
    if extracted_frames:
        print("\nGenerating descriptions for extracted frames using Qwen...")
        ...
        for i, pil_image in enumerate(extracted_frames):
            try:
                ...
                all_frame_descriptions_qwen.append(description)
            except Exception as e:
                print(f"Error processing frame {i}: {e}")

        if all_frame_descriptions_qwen:
            print("\n--- Generating Video Summary (Qwen) ---")
            combined_frame_texts = "\n".join(all_frame_descriptions_qwen)
            ...
            try:
                ...
                video_summary_qwen = processor.batch_decode(
                    generated_ids_trimmed, skip_special_tokens=True, clean_up_tokenization_spaces=False
                )[0]
                
                print("Based on the extracted frames, the video appears to show the following:\n")
                print(video_summary_qwen)
                return video_summary_qwen
            except Exception as e:
                print(f"Error generating summary with Qwen: {e}")
                print("Fallback: Concatenating frame descriptions...")
                video_summary_qwen = " ".join(all_frame_descriptions_qwen)
                print("Based on the extracted frames, the video appears to show the following:\n")
                print(video_summary_qwen)
                return None
        else:
            print("No frames were described by Qwen.")
            return None
    else:
        ...
        if extracted_frames:
            ...
            if all_frame_descriptions_qwen:
                ...
                try:
                    ...
                    print(f"Error generating summary for row {row_id} with Qwen: {e}")
                    balanced_split_desc.loc[row_id, 'predicted_summary'] = "Error: " + str(e)
                    balanced_split_desc.to_csv(SPLIT_DESC_CSV_SAVE_PATH, index=False)
                    continue
            else:
                print(f"No frames were described by Qwen for row {row_id}.")
